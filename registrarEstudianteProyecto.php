...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO proyecto_estudiante (id_proyecto, id_estudiante) VALUES (:id_proyecto, :id_estudiante)");
...
<div class="container">
    <div class="card">
        <div class="card-header">
            <h2>Asignar Estudiante a Proyecto</h2>
        </div>
        <div class="card-body">
            <form action="registrarEstudianteProyecto.php" method="POST">
                <div class="mb-3">
                    ...
                    <select class="form-select" id="id_proyecto" name="id_proyecto" required>
                        <?php foreach ($proyectos as $proyecto): ?>
                            <option value="<?php echo $proyecto['codigo']; ?>"><?php echo "ID: {$proyecto['codigo']} - Código: {$proyecto['titulo']}"; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select" id="id_estudiante" name="id_estudiante" required>
                        <?php foreach ($estudiantes as $estudiante): ?>
                            <option value="<?php echo $estudiante['id']; ?>"><?php echo "ID: {$estudiante['id']} - Nombre: {$estudiante['nombre']}"; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
